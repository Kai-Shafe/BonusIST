...
pathToFile <- paste(nd[[2]], "/", sep = "")
raw.file.paths <- raw.files %>% mutate(filepath = pathToFile)
dfcn = paste(raw.file.paths[1, "filepath"], raw.file.paths[10, "filename"], sep = "")
...
barplot(sortedDisplayGroup, main = "Total trips done (weekday/month)", xlab = "Weekday", ylab = "Trips", col = heat.colors(length(monthSort)), beside = TRUE ,legend = rownames(sortedDisplayGroup), names.arg = weekdaySort)
```
